...
export class SassClient {
  ...
  async uploadFile(myId: string, filename: string, file: File) {
    ...
    return this.client.storage.from(STORAGE_BUCKET).upload(filename, file);
  }

  async getFiles(myId: string) {
    return this.client.storage.from(STORAGE_BUCKET).list(myId);
  }

  async deleteFile(myId: string, filename: string) {
    filename = myId + "/" + filename;
    return this.client.storage.from(STORAGE_BUCKET).remove([filename]);
  }
